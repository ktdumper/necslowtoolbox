...
class Exploit:
    ...
    def run(self):
        ...
        desc = bytearray(self.dev.ctrl_transfer(0x80, 0x06, 0x0200, 0x00, 0x40))
        ...
        payload = PayloadBuilder("dumper.c").build(base=0x80000000, desc=",".join(hex(x) for x in desc), dump_addr=self.args.dump_addr)
        ...
        while True:
            try:
                time.sleep(0.5)
                self.dev.ctrl_transfer(0x80, 0x06, 0x0200, 0x00, 0x40)
                break
